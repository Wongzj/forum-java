  ...
  <div class="article-editor article-box" :style="{height: containerHeight}">
    ...
    <div class="article-box-title">
      <div class="article-box--title-home">
        <Button @click="goHome" type="info">Back to Home</Button>
      </div>
      <input v-model="article.title" class="article-box-title-input" placeholder="Enter article title here"/>
    </div>
    <mavon-editor
      class="article-box-content"
      language="en"
      placeholder="Enter the article content here, in markdown format."
      :style="{zIndex:0}"
    ...
    <div class="article-box-foot">
      ...
      <div class="article-box-foot-tag">
        ...
        <Button type="dashed" size="small" @click="showTagSelectModel">Add Tag</Button>
      </div>
...
export default {
  ...
  methods: {
    ...
    updateArticle () {
      this.postArticle('Update completed', () => {})
    },
    saveArticle () {
      this.postArticle('Saved successfully', () => {
        location.href = '/'
      })
    },
    ...
    preCheck () {
      ...
      if (this.article.title.length > 40) {
        this.$Message.error('Title cannot exceed 40 characters')
        return false
      }
      ...
      if (!this.article.typeId) {
        this.$Message.error('Category cannot be empty')
        return false
      }
      if (!this.article.tags || this.article.tags.length === 0) {
        this.$Message.error('Label cannot be empty')
        return false
      }
      return true
    },
  ...
  created () {
    ...
    if (this.$route.params.id) {
      ...
      document.title = 'Write Articles'
      this.article.markdownContent = `# 1 First-level Title
